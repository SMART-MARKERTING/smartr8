...
const DEFAULT: FS = { step:1,firstName:"",lastName:"",address:"",city:"",stateCode:"",zip:"",homeValue:"",homeValueDraft:"",mortgageBalance:"",mortgageBalanceDraft:"",currentRate:"",currentRateDraft:"",primaryGoal:"",creditScore:"",dob:"",email:"",phone:"",consent:false };
...
export default function RateReductionFunnel() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await submitLead({ funnel:"rate-reduction", subjectLine:"New RATE REDUCTION lead from smartr8.com", loanPurpose:"Rate Reduction Refinance", firstName:st.firstName, lastName:st.lastName, email:st.email, phone:st.phone, address:st.address, city:st.city, state:st.stateCode, zip:st.zip, homeValue:st.homeValue, mortgageBalance:st.mortgageBalance, creditScore:st.creditScore, dob:st.dob, additionalFields:{ currentRate:st.currentRate, primaryGoal:st.primaryGoal } });
      ga4.trackLead(); sessionStorage.removeItem(SESSION_KEY);
      setLocation(`/apply/rate-reduction/whats-next?name=${encodeURIComponent(st.firstName)}`);
