...
import { computeScenarios, makeDefaultInputs, type WorksheetInputs } from "@/lib/worksheetCalc";
...
function SendToClientModal({
  open, onOpenChange, inputs, results,
}: {
  open: boolean;
  ...
  const [clientFirstName, setClientFirstName] = useState(inputs.clientFirstName || "");
  const [clientLastName, setClientLastName] = useState(inputs.clientLastName || "");
  ...
  async function handleSend() {
    ...
    setSending(true);
    try {
      // Inject lead's name into PDF so "Prepared For" shows it
      const pdfInputs: WorksheetInputs = {
        ...inputs,
        clientFirstName: clientFirstName.trim(),
        clientLastName: clientLastName.trim(),
      };
      const base64 = await getWorksheetPdfBase64(pdfInputs, results);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          <DialogTitle>Send Worksheet to a Client</DialogTitle>
          <DialogDescription>
            Generates the PDF from current numbers and emails it directly to the client.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 pt-2">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <Input placeholder="Jane" value={clientFirstName} onChange={(e) => setClientFirstName(e.target.value)} disabled={sending} />
            </div>
            <div className="space-y-1">
              <Label>Last Name</Label>
              <Input placeholder="Smith" value={clientLastName} onChange={(e) => setClientLastName(e.target.value)} disabled={sending} />
            </div>
          </div>
          <div className="space-y-1">
            <Label>Client Email <span className="text-destructive">*</span></Label>
            <Input type="email" placeholder="client@example.com" value={clientEmail} onChange={(e) => setClientEmail(e.target.value)} disabled={sending} />
          </div>
          <Button
          ...
          >
            {sending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Sending…</> : <><Send className="mr-2 h-4 w-4" /> Send to {clientFirstName || "Client"}</>}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
...
export default function WorksheetInternal() {
  ...
  const [inputs, setInputs] = useState<WorksheetInputs>(() => makeDefaultInputs());
  ...
  async function handleDownloadPdf() {
    setPdfLoading(true);
    try {
      const name = `${inputs.clientFirstName} ${inputs.clientLastName}`.trim().replace(/\s+/g, "-") || "Client";
      ...
      <div className="min-h-screen bg-background flex flex-col print:bg-white">
        ...
        <main className="flex-1 container mx-auto max-w-7xl px-4 py-8 print:p-0">
          <div className="print:hidden mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground">
                Build a personalized worksheet for a client, then download or send the PDF.
              </p>
            </div>
          ...
          <div className="flex flex-col lg:flex-row gap-8 print:block">
            <aside className="lg:w-80 xl:w-96 shrink-0 print:hidden">
              <div className="sticky top-4 bg-card border rounded-lg p-5 shadow-sm max-h-[calc(100vh-2rem)] overflow-y-auto">
